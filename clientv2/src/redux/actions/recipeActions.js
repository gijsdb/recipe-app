...
export const addRecipe = ({ Title, AddedBy }) => (
  dispatch,
  getState,
) => {
...
export const retrieveMyRecipes = (AddedBy) => (
  dispatch,
  getState,
) => {
  const token = getState().authReducer.token;
...
export const setMethod = (method, recipeId) => (
  dispatch,
  getState,
) => {
  const token = getState().authReducer.token;
  ...
  axios
    .post(
      'http://localhost:8000/api/recipes/add/method',
      body,
      config,
    )
    .then((res) => {
...
export const setIngredients = (ingredientsList, recipeId, serves) => (
  dispatch,
  getState,
) => {
  const token = getState().authReducer.token;
  ...
  axios
    .post(
      'http://localhost:8000/api/recipes/add/ingredients',
      body,
      config,
    )
    .then((res) => {
